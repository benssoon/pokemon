...
class Game(object):	# Inheriting from object is from Python 2.x and indicates a new-style class as opposed to old-style. It is not necessary in Python 3.x as all classes inherit from object implicitly. However, it may be a good idea to leave it in if your code is ever likely to be distributed, because if someone tried to use it in Python 2.x thinking it was old-style, there would be some serious issues. Although this is the case, it does seem like that shouldn't ever happen if I define Python 3.x with the #! at the top. Anyway, in the future, I will leave this out (for now) since my code is not distributed.
	"""
	...
	def run_logic(self):
		...
		if not self.game_over:
			...
			"""
			Check if enemies have been hit. If any weapon hits an enemy, do damage to the enemy equivalent to the damage value of the weapon.
			The values of the dictionary are lists of items that have collided with the keys,
			thus it is necessary to loop through each value to get the damage accumulated from each weapon.
			If the enemy's health reaches 0, kill it.
			"""
